### This is from https://github.com/JuliaAlgebra/HypersurfaceRegions.jl/blob/main/src/path_tracking.jl

# define the gradient ascend ode
function set_up_ode(r)

    function ode_log!(u, dy, y, p, t)
        n = length(u)
        HC.evaluate!(u, r, @view y[:, 1])
        for i = 1:n
            dy[i] = real(u[i])
        end
    end
...
function finding_nearest_point(v, list_of_vectors)
    ...
    distances = [LinearAlgebra.norm(v - x) for x in list_of_vectors]
